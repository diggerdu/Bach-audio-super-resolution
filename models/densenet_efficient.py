...
class _UpTransition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.add_module('transposed_conv', nn.ConvTranspose2d(num_input_features, num_output_features,
...
class _DenseBlock(nn.Container):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, storage_size=1024):
        ...
        for i in range(num_layers):
            print('input_features%d', num_input_features+i*growth_rate)
...
class DenseNetEfficient(nn.Module):
    ...
    """
    def __init__(self, growth_rate=12, block_config=(4, 4, 4, 4), compression=0.5,
                 num_init_features=32, bn_size=4, drop_rate=0, avgpool_size=8,
                 num_classes=10):
        ...
        self.blocklen = len(block_config)
        self.downfeature_list = [] # a list recording downsamplinglayers' features(before_denseblock)
        self.feature_eachlayer_list =[]
        curr_features = num_init_features
        for i, num_layers in enumerate(block_config):
            self.downfeature_list.append(curr_features)
            self.feature_eachlayer_list.append(growth_rate * num_layers)
            curr_features += (growth_rate * num_layers)
        self.downfeature_list.reverse()
        self.feature_eachlayer_list.reverse()

        # # get the upsampling list
        # for i, num_layers in enumerate(reversed(block_config)):
        #     if i != 0:
        #         curr_features_afterupsampling = self.downfeature_list[len(block_config) - 1 - i] \
        #                                         + 2 * num_layers * growth_rate + block_config[len(
        #             block_config) - i] * growth_rate
        #         self.upfeature_list.append((curr_features, curr_features_afterupsampling))
        #         curr_features = curr_features_afterupsampling + growth_rate * num_layers
        # # TODO
        # print('upfeaturelist is ', self.upfeature_list)
        # print('downfeaturelist is ', self.downfeature_list
        #       )
        ...
        #         trans = _DownTransition(num_input_features=num_features,
        #                                 num_output_features=int(num_features
        #                                                     * compression))
        #         self.features.add_module('transition%d' % (i + 1), trans)
        #         num_features = int(num_features * compression)

        # essential part(skip connection and densenet)
        for i, num_layers in enumerate(reversed(block_config)):
            if i != len(block_config) -1:
                if i != 0:
                    # normal
                    self.model = SkipConnectionBlock(num_layers=num_layers,
                                                     down_features=self.downfeature_list[i],
                                                     up_features=self.downfeature_list[i] + num_layers*growth_rate
                                                     + self.feature_eachlayer_list[i-1],
                                                     bn_size=bn_size,
                    ...
                    self.model = SkipConnectionBlock(num_layers=num_layers,
                                                     down_features=self.downfeature_list[i],
                                                     up_features=self.downfeature_list[i],
                                                     bn_size=bn_size,
                                                     growth_rate=growth_rate,
                                                     drop_rate=drop_rate,
                                                     innermost=True)
        ...
        self.features.add_module('densenet_part', self.model)
        num_features = self.downfeature_list[self.blocklen-1]+block_config[self.blocklen-1]*growth_rate \
                      +self.feature_eachlayer_list[self.blocklen-2]+self.feature_eachlayer_list[self.blocklen-1]
        # final_layer without denseblock
        # final_layer = nn.Sequential(OrderedDict([('norm_final', nn.BatchNorm2d(num_features)),
        #                                              ('relu_final', nn.ReLU(inplace=True)),
        #                                              ('last_conv', nn.Conv2d(num_features, 1, kernel_size=1)), ]))
...
class SkipConnectionBlock(nn.Module):
    def __init__(self,
        ...
        self.outtermost = outtermost

        if innermost:
            down = [_DownTransition(num_input_features=down_features,num_output_features=down_features,)]
            innerblock = [_DenseBlock(num_layers=num_layers,
                                      num_input_features=down_features,
                                      bn_size=bn_size, growth_rate=growth_rate,
                                      drop_rate=drop_rate)]
            up = [_UpTransition(num_input_features=down_features+num_layers*growth_rate,
                                num_output_features=num_layers*growth_rate)]
            model = down+innerblock+up

        elif outtermost:
            down = [_DenseBlock(num_layers=num_layers, num_input_features=down_features, bn_size=bn_size,
                                growth_rate=growth_rate, drop_rate=drop_rate)
                    ]
            up = [_DenseBlock(num_layers=num_layers, num_input_features=up_features, bn_size=bn_size,
                              growth_rate=growth_rate, drop_rate=drop_rate)
                  ]
            ...
            down = [_DownTransition(num_input_features=down_features, num_output_features=down_features),
                    _DenseBlock(num_layers=num_layers, num_input_features=down_features, bn_size=bn_size,
                                growth_rate=growth_rate, drop_rate=drop_rate),
                    ]
            up = [_DenseBlock(num_layers=num_layers, num_input_features=up_features, bn_size=bn_size,
                              growth_rate=growth_rate, drop_rate=drop_rate),
                  _UpTransition(num_input_features=up_features+growth_rate*num_layers,
                                num_output_features=num_layers * growth_rate),
                  ]
